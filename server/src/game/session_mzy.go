...
type JS_SDKLoginData_MZY struct {
	Sid      string `json:"sid"`
	Username string `json:"username"`
}

type JS_SDKData_MZY struct {
	Uid string `json:"uid"`
}

type JS_SDKBody_MZY struct {
	Id    string         `json:"id"`
...
// 安卓登录-拇指游玩登录
func (self *Session) SDKReg_MZY(token string, serverid int, username string, ctrl string) *San_Account {
	...
	h1 := md5.New()
	LogInfo("New MZY User:", username, token)
	h1.Write([]byte(username + token))
