...
func BeginningOfMinute() time.Time {
	return NewTimeUtil(TimeServer()).BeginningOfMinute()
}

// BeginningOfHour beginning of hour
func BeginningOfHour() time.Time {
	return NewTimeUtil(TimeServer()).BeginningOfHour()
}

// BeginningOfDay beginning of day
func BeginningOfDay() time.Time {
	return NewTimeUtil(TimeServer()).BeginningOfDay()
}

// BeginningOfWeek beginning of week
func BeginningOfWeek() time.Time {
	return NewTimeUtil(TimeServer()).BeginningOfWeek()
}

// BeginningOfMonth beginning of month
func BeginningOfMonth() time.Time {
	return NewTimeUtil(TimeServer()).BeginningOfMonth()
}

// BeginningOfQuarter beginning of quarter
func BeginningOfQuarter() time.Time {
	return NewTimeUtil(TimeServer()).BeginningOfQuarter()
}

// BeginningOfYear beginning of year
func BeginningOfYear() time.Time {
	return NewTimeUtil(TimeServer()).BeginningOfYear()
}

// EndOfMinute end of minute
func EndOfMinute() time.Time {
	return NewTimeUtil(TimeServer()).EndOfMinute()
}

// EndOfHour end of hour
func EndOfHour() time.Time {
	return NewTimeUtil(TimeServer()).EndOfHour()
}

// EndOfDay end of day
func EndOfDay() time.Time {
	return NewTimeUtil(TimeServer()).EndOfDay()
}

// EndOfWeek end of week
func EndOfWeek() time.Time {
	return NewTimeUtil(TimeServer()).EndOfWeek()
}

// EndOfMonth end of month
func EndOfMonth() time.Time {
	return NewTimeUtil(TimeServer()).EndOfMonth()
}

// EndOfQuarter end of quarter
func EndOfQuarter() time.Time {
	return NewTimeUtil(TimeServer()).EndOfQuarter()
}

// EndOfYear end of year
func EndOfYear() time.Time {
	return NewTimeUtil(TimeServer()).EndOfYear()
}

// Monday monday
func Monday() time.Time {
	return NewTimeUtil(TimeServer()).Monday()
}

// Sunday sunday
func Sunday() time.Time {
	return NewTimeUtil(TimeServer()).Sunday()
}

// EndOfSunday end of sunday
func EndOfSunday() time.Time {
	return NewTimeUtil(TimeServer()).EndOfSunday()
}

//! today timestamp
func GetTodayTime(hour int, minute int, second int) int64 {
	y, m, d := TimeServer().Date()
	return time.Date(y, m, d, hour, minute, second, 0, TimeServer().Location()).Unix()
}

// Parse parse string to time
func Parse(strs ...string) (time.Time, error) {
	return NewTimeUtil(TimeServer()).Parse(strs...)
}

// ParseInLocation parse string to time in location
func ParseInLocation(loc *time.Location, strs ...string) (time.Time, error) {
	return NewTimeUtil(TimeServer().In(loc)).Parse(strs...)
}

// MustParse must parse string to time or will panic
func MustParse(strs ...string) time.Time {
	return NewTimeUtil(TimeServer()).MustParse(strs...)
}
...
func MustParseInLocation(loc *time.Location, strs ...string) time.Time {
	return NewTimeUtil(TimeServer().In(loc)).MustParse(strs...)
}

// Between check now between the begin, end time or not
func Between(time1, time2 string) bool {
	return NewTimeUtil(TimeServer()).Between(time1, time2)
}
