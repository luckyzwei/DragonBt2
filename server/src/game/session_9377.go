...
type JS_SDKLoginData_9377 struct {
	Sid      string `json:"sid"`
	Username string `json:"username"`
}

type JS_SDKData_9377 struct {
	Uid string `json:"uid"`
}

type JS_SDKBody_9377 struct {
	Id    string          `json:"id"`
	State JS_SDKState     `json:"state"`
	Data  JS_SDKData_9377 `json:"data"`
}

// 安卓登录-拇指游玩登录
func (self *Session) SDKReg_9377(token string, serverid int, username string, ctrl string) *San_Account {
	log.Println("9377 login:", username, token, serverid)
	...
	h1 := md5.New()
	LogInfo("New 9377 User:", username, token)
