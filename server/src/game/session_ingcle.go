...
type JS_SDKLoginData_KoYou struct {
	Sid      string `json:"sid"`
	Username string `json:"username"`
}

type JS_SDKData_KoYou struct {
	Uid string `json:"uid"`
}

type JS_SDKBody_KoYou struct {
...
// 安卓登录-云客登录
func (self *Session) SDKReg_Ingcle(token string, serverid int, username string, ctrl string) *San_Account {
	log.Println(token)
	channelId := 1109
	gameId := 258
	...
	url := "http://usdk.xxxxxxx.com/cp/cp/check"
